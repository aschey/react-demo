...
export const Board: React.FC<{}> = () => {
  ...
  const renderSquare = (i: number): JSX.Element => {
    // Here we pass a function as a property to the child component
    // Since Square's click event relies on state that belongs to the parent, we let the parent do the heavy lifting
    return <Square value={squares[i]} onClick={() => handleClick(i)} />;
  };

  return (
    <div>
      <div className="status">{/* add status message */}</div>
